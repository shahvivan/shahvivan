...
const REQ = [
  ...
  ["Finishing means the phone works too", "P", "Completion requires both halves: the three actions done for real, and the device actually set up. Permissions are read back rather than trusted, so a guard cannot confirm a gate they did not complete. NFC and background tracking are still taken on the guard’s word, because it is not yet confirmed the app can read them; if it can, they should be read back too. A guard who finishes and then cannot scan a checkpoint has not been onboarded."],
...
[
  ...
  "Which permissions and grant levels does the app need (e.g. location “all the time”)? Reading permission state back is assumed, since the Location screen already shows a “Permissions needed” status and a settings link. The open question is whether NFC and background-tracking state can be read the same way, so all three gates can be verified rather than two of them asked. Can setup also stop the phone’s automatic permission removal for unused apps?",
