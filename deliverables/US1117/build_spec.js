...
const REQ = [
  ...
  ["Setup is per phone, training is per guard", "X", "Confirmed: guards share handsets, each with their own login. So the phone being ready cannot be recorded against the guard. A completion flag on the users table marks the guard, so the same guard on a second phone is treated as set up on a phone that never went through the gates. Gate confirmations need their own per-device record; an unrecognised handset replays the three gates. This is the one open item that can still defeat the promise that the phone is ready."],
  ["Finishing means the phone works too", "P", "Completion requires both halves: the three actions done for real, and the device set up. The app can detect most missed permissions and will not advance past them. Three it cannot see — location at “allow all the time”, background activity, physical activity — so those are guided and then trusted."],
...
[
  ...
  "Where do the per-device gate confirmations live? A flag on the users table marks the guard, not the phone, and guards share phones.",
