...
const AC = [
  ...
  ["AC-18", "FR-23", "Given the background-tracking gate, when the guard follows the link to the in-app setting, switches it on and returns, then they confirm and the flow advances. Background activity is one of the three states the app cannot read, so what is observed here is the confirmation, not the setting."],
  ["AC-19", "FR-26", "Given Talk produces nothing repeatedly, then the guard is told the likely causes — the microphone prompt was refused, or the app has not restarted since it was granted — rather than shown the same retry prompt again."],
...
const UAT = [
  ...
  ["UAT-26", "AC-04 / AC-17 / AC-18", "Confirm each gate with the setting still off. (negative)", "A readable setting stops the gate advancing. The three unreadable ones — location at “allow all the time”, background activity, physical activity — are accepted, and re-checked next session (FR-32)."],
  ["UAT-27", "AC-19", "Press Talk repeatedly having refused the microphone prompt. (negative)", "Both likely causes named; no endless identical retry."],
