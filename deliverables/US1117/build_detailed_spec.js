...
const FR = [
  ...
  ["FR-28", "P", "Retry floor and release", "No step may trap a guard indefinitely. After three failed attempts at a spoken step, typing the answer is offered. If a step still cannot be completed, or isolation stays unavailable, the guard is released to the home screen with completion unmarked and the stage saved. Being un-onboarded is a reporting problem; being locked out of the app mid-shift is a safety one."],
  ["FR-29", "P", "A real incident reported inside practice", "A practice banner stays visible for the whole exercise, not only on the finished report card. If what the guard reports diverges materially from the suggested incident, SAM asks whether this is real before writing it, and a yes routes to normal reporting. Without this, a genuine incident reported during practice is flagged training and hidden from supervisors by the very mechanism meant to protect them. The risk persists after go-live through the refresher."],
  ["FR-30", "X", "Launch blocked without the filter", "At launch, onboarding checks server-side that the exclusion filter is present and verified, and refuses to run if it is not. The rollout switch alone is not sufficient protection: activities send supervisor alerts, move KPIs and create Action Tracker items today, and a dispatched alert cannot be recalled. A capability check makes the dependency a mechanism rather than a release convention."],
...
const AC = [
  ...
  ["AC-04", "FR-06", "Given the permissions gate, when the guard follows the link, enables the permissions and returns, then they confirm with a tap and the flow continues; the link opened the phone’s settings page for SAM OnSite from inside onboarding and returned there, offering no route into the app’s ordinary navigation. Confirmation works before microphone permission is granted. If a permission cannot be granted at all, the guard reaches a support screen and is not trapped."],
  ...
  ["AC-08", "FR-11", "Given SAM has named the change and the guard long-presses the message they sent and saves it with the location added, then the final report contains both “MacBook” and “reception”."],
  ...
  ["AC-22", "FR-28", "Given three failed attempts at a spoken step, then typing is offered; and given a step that still cannot be completed, then the guard reaches the home screen with completion unmarked and the stage saved."],
  ["AC-23", "FR-29", "Given a practice run, then a practice banner is visible at every step; and given a report that diverges materially from the suggested incident, then SAM asks whether it is real before writing it, and a yes routes to normal reporting."],
  ["AC-24", "FR-30", "Given the exclusion filter is absent or unverified, when onboarding is launched in any environment, then it refuses to start and reports why."],
...
const UAT = [
  ...
  ["UAT-07", "AC-06", "Ask SAM a question.", "SAM answers; nothing live, and any logged record carries the training flag."],
  ...
  ["UAT-27", "AC-04", "Confirm the permissions gate with a permission still switched off. (negative)", "Confirmation is accepted per MVP behaviour; the miss surfaces at first use, except the microphone, which is caught by UAT-29. Recorded as the rationale for FR-24."],
  ...
  ["UAT-30", "AC-22", "Fail a spoken step three times. (negative)", "Typing offered; and on continued failure, home screen reached with completion unmarked and stage saved."],
  ["UAT-31", "AC-23", "Report an incident unrelated to the suggested one during practice. (negative)", "Practice banner visible throughout; SAM asks whether it is real; a yes routes to normal reporting rather than writing a training-flagged report."],
  ["UAT-32", "AC-24", "Launch onboarding with the exclusion filter absent. (negative)", "Refuses to start and reports why."],
...
const TRACE = [
  ...
  ["Real incident inside practice", "FR-29", "AC-23", "UAT-31", "Asked before writing; routed out"],
  ["Launch blocked without the filter", "FR-30", "AC-24", "UAT-32", "Refuses to start"],
  ["Monitoring transparency", "FR-31", "AC-25", "—", "Copy states what a supervisor sees"],
  ...
  ["Ask a question (nothing live)", "FR-08", "AC-06", "UAT-07", "Answer given; nothing live, record training-flagged"],
...
[
  ...
  "Then the same fix by hand. SAM names the change it wants, the guard long-presses the message they just sent, adds that it happened at reception, and saves. The final report shows both “MacBook” and “reception”.",
...
kids.push(table([1500, 4074, 4074], ["Stage", "SAM says", "Guard does"], [
  ["Welcome", "“Hello, I’m SAM. From today you talk and I write it up. Give me a few minutes and I’ll show you how.”", "Reads, then taps Start."],
  ["Practice context", "“Nothing here counts. Whatever we write, I mark as training, and nobody else sees it. If you have to stop, I’ll remember where you were.”", "Taps Continue."],
  ["Permissions", "“First, permissions. Without them I can’t hear you or see where you are. This link takes you to your phone’s settings for SAM OnSite. Location needs to be All the time, the rest just switch on. Then come back and tap Done.”", "Follows the link, enables each permission at the named level, returns, taps Done."],
  ["NFC", "“Now NFC. It’s what reads the tags at the checkpoints, so without it your rounds won’t register. Switch it on through here and tap Done when you’re back.”", "Enables NFC, returns, taps Done. Skipped if the phone has no NFC chip."],
  ...
  ["Ask a question", "“Press Talk and ask me something. Ask me how you report an incident.”", "Presses Talk and asks. The first transcription doubles as the mic check. SAM answers with the prepared answer; nothing live results."],
  ...
  ["Voice fix", "“If you get something wrong, just tell me. Tell me the Dell should be a MacBook.” … “Changed. Same report.”", "Says the Dell should be a MacBook. Same report updates."],
  ["Manual fix", "“You can fix it by hand too. Press and hold the message you sent me, and add that it happened at reception.”", "Long-presses that message, adds “at reception”, saves."],
