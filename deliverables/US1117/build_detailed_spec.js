...
kids.push(table([2600, 7048], ["Situation", "Behaviour"], [
  ["A setting is still off after the guard confirms", "Most misses are caught: the app can detect a permission that should be on and is off, so those gates do not advance (FR-24). Three cannot be checked — location at “allow all the time”, background activity, and physical activity — and are guided, then taken on the guard’s word. Location is the one that bites: granted only “while using”, it silently defeats the background-tracking gate the guard has just passed, with no visible failure. The closing screen names what was switched on so the guard has a last chance to catch it. Anything the guard declares ungrantable reaches a support screen."],
...
const valRef = "val";
[
  "Where do the per-device gate confirmations live? A completion flag on the users table marks the guard, not the phone, so it does not answer this. Guards share handsets, so without a per-device record a guard is treated as set up on a phone that never went through the gates.",
