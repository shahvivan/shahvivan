 ...
 * Out:  SAM_OnSite_Guard_Onboarding_Spec_short.docx
 *
 * Design goals:
...
kids.push(callout([
  [t("In one minute.  ", { bold: true, color: TEALD }), t("At first sign-in SAM introduces itself, checks the phone is set up (location, NFC, background tracking, battery) and teaches three actions — ask, report, correct — on a made-up incident. Roughly three to five minutes, mandatory, saves progress, done only when each step is genuinely done. It replaces the deck’s app-setup slides and absorbs its tips. The practice report is written already flagged and under its own scenario, which keeps it out of the incident list and stops any alert. Four surfaces do not inherit that flag and are named in section 02; until they are closed this runs in DEV/UAT — the internal test systems.")],
]));
kids.push(gap(30));
...
[
  ...
  "Four setup gates, each reached by a link in the chat: location (set to Allow all the time in settings), NFC, background tracking, which sits in SAM OnSite’s own settings, and battery optimisation, which Android uses to shut background tracking down quietly unless the app is exempt. SAM reads all four and raises only what is off, so a guard on a phone already in use walks straight past them. A handset with no NFC chip skips that gate instead of failing on it. Microphone and camera are not gated; Android prompts for those when first used. Once everything is enabled the guard has to be guided back: restart the app and open the existing chat rather than start a new one, or a Continue onboarding button sits in the open space until onboarding is marked done. A new chat cannot be started before onboarding is complete, except under the urgency override.",
  ...
  "The practice report. SAM asks for it the way the guard would tell a colleague and offers one: a Dell laptop is missing. One report is written, flagged and under the training scenario from the moment the record is created.",
  "The guard is told to fix it by hand: press and hold their own report message, which SAM marks on screen, and edit it. Only a guard’s own written message can be edited, and only once it has saved, so the step waits for the report to appear.",
...
/* ---- 02 What must not go wrong ---- */
kids.push(eyebrow("02", "What must not go wrong"));
kids.push(subhead("PRACTICE DATA REACHING LIVE OPERATIONS"));
kids.push(p([
  t("A pretend stolen laptop must never wake a real supervisor. Two things stop it, and both already travel the whole way from the app to Pronect. The flag keeps the report out of the incident list. Its scenario keeps it quiet, because alerts are addressed by scenario and nobody is subscribed to a training one. Both have to be set as the record is written. Archiving it a moment later is too late: the alert goes out on creation, and a dispatched alert cannot be recalled. Four surfaces do not inherit the flag and each needs its own fix — the reporter name lookup, photos handed out with list data, action plans that copy the report text, and the map inside SAM’s own chat. A second Pronect front-end has not been checked. Until those are closed this stays in DEV/UAT, and a check on the server should refuse to start onboarding without them. Flagged records are hidden, not deleted; clearing them is a separate job with no date."),
]));
...
const OUT = path.join(__dirname, "SAM_OnSite_Guard_Onboarding_Spec_short.docx");
